...
function render() {
	...
	document.querySelectorAll('[data-item-image]').forEach((input) => input.addEventListener('change', async () => { const file = input.files[0]; if (!file) return; if (typeof supabaseClient === 'undefined') return notify('Image storage is not configured'); const [categoryIndex, itemIndex] = input.dataset.itemImage.split('-').map(Number); if (!file.type.startsWith('image/')) return notify('Please choose an image file'); if (file.size > 5 * 1024 * 1024) return notify('Image must be smaller than 5 MB'); const path = `${client.slug}/${Date.now()}-${file.name.replace(/[^a-z0-9._-]/gi, '-')}`; const { error } = await supabaseClient.storage.from('menu-images').upload(path, file, { upsert: true, contentType: file.type }); if (error) return notify(`Image upload failed: ${error.message}`); client.categories[categoryIndex].items[itemIndex].image_url = supabaseClient.storage.from('menu-images').getPublicUrl(path).data.publicUrl; try { await saveClients(); render(); notify('Image uploaded'); } catch (error) { notify(error.message); } }));
