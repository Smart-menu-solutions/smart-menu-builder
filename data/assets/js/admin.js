...
function render() {
	...
	$('#languageEditor').innerHTML = languageOrder.map((code, index) => {
		...
		return `<div class="language-row${checked && !hasTranslation ? ' lang-missing' : ''}"><label class="language-check"><input type="checkbox" name="language" value="${code}"${checked ? ' checked' : ''}><span>${escapeHtml(entry?.label || code)}${isMain ? ' (main)' : ''}</span></label><span class="language-move"><button type="button" class="move-language" data-move-language="up-${index}" title="Move language up" aria-label="Move language up" ${index === 0 ? 'disabled' : ''}>↑</button><button type="button" class="move-language" data-move-language="down-${index}" title="Move language down" aria-label="Move language down" ${index === languageOrder.length - 1 ? 'disabled' : ''}>↓</button></span></div>`;
	}).join('');
