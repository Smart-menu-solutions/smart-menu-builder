	...
	return {
		...
		categories: Array.isArray(client.categories) ? client.categories.map((category) => ({
			...
			items: Array.isArray(category.items) ? category.items.map((item) => ({
				...
				// Article number in the restaurant's own till - shown only on the
				// ServiceHub cashier screen (staff-access), never to guests.
				posNumber: String(item.posNumber || '').trim().slice(0, 20)
			})) : []
		})) : [],
...
function render() {
	...
	// Till number field only for restaurants with Smart ServiceHub - it's only
	// used on the ServiceHub cashier screen.
	const hasPosField = !!client.smartservice_hub_enabled;
	const posLabel = escapeAttr(editorText.posNumber);
	const posHint = escapeAttr(editorText.posNumberHint);
	const removeDishLabel = escapeAttr(editorText.removeDish);
	$('#categoryEditor').innerHTML = client.categories.map((category, categoryIndex) => `<div class="category-block"><div class="category-top"><input data-category-name="${categoryIndex}" value="${escapeAttr(category.name)}" aria-label="${sectionNameAria}"><span class="category-move"><button type="button" class="move-category" data-move-category="up-${categoryIndex}" title="${moveSectionUp}" aria-label="${moveSectionUp}">↑</button><button type="button" class="move-category" data-move-category="down-${categoryIndex}" title="${moveSectionDown}" aria-label="${moveSectionDown}">↓</button></span><button type="button" class="remove-button" data-remove-category="${categoryIndex}" title="${removeSectionLabel}">×</button></div>${imageControl('category', categoryIndex, category.image)}${courseTypeControl(category, categoryIndex)}<div class="category-items">${category.items.map((item, itemIndex) => `<div class="item-block"><div class="item-row${hasPosField ? ' has-pos' : ''}"><input data-item-name="${categoryIndex}-${itemIndex}" value="${escapeAttr(item.name)}" placeholder="${dishNameLabel}" aria-label="${dishNameLabel}"><input data-item-description="${categoryIndex}-${itemIndex}" value="${escapeAttr(item.description)}" placeholder="${dishDescriptionLabel}" aria-label="${dishDescriptionAria}"><input data-item-price="${categoryIndex}-${itemIndex}" value="${escapeAttr(item.price)}" placeholder="0.00" aria-label="${priceLabel}">${hasPosField ? `<input data-item-pos="${categoryIndex}-${itemIndex}" value="${escapeAttr(item.posNumber || '')}" placeholder="${posLabel}" title="${posHint}" aria-label="${posLabel}" maxlength="20">` : ''}<button type="button" class="remove-button" data-remove-item="${categoryIndex}-${itemIndex}" title="${removeDishLabel}">×</button></div>${imageControl('item', `${categoryIndex}-${itemIndex}`, item.image)}${itemTagsControl(item, categoryIndex, itemIndex)}</div>`).join('')}</div><button type="button" class="add-item" data-add-item="${categoryIndex}">${escapeHtml(editorText.addDish)}</button></div>`).join('');
