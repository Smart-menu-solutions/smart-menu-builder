...
//
// Uses real Stripe Subscription Items (not a one-off charge we compute
// ourselves) so Stripe prorates the charge for the rest of the current
// year automatically, and the add-on then renews correctly with the base
// plan every year after with no extra bookkeeping here.
...
// One entry per purchasable add-on. priceId comes from a real, persistent
// Stripe Price (subscriptionItems.create can't spontaneously create one the
// way Checkout's price_data can) - see the plan's Stripe setup step.
const ADDONS: Record<string, { label: string; menusColumn: string; itemIdColumn: string; priceId: string }> = {
	smart_food_match: {
...
async function lookupSubscription(token: string) {
	return supabase
		.from('subscriptions')
		.select('id, status, menu_slug, stripe_subscription_id, smart_food_match_item_id, analytics_reports_item_id, customers(stripe_customer_id, contact_name, email), menus(name, smart_food_match_enabled, analytics_reports_enabled)')
...
async function handleGet(url: URL) {
	...
	const menu = subscription.menus as { smart_food_match_enabled: boolean; analytics_reports_enabled: boolean } | null;
	return json({
		status: subscription.status,
		addons: Object.entries(ADDONS).map(([key, addon]) => ({
			key,
			label: addon.label,
			priceLabel: '€5.00 / year',
			active: !!menu?.[addon.menusColumn as 'smart_food_match_enabled' | 'analytics_reports_enabled']
		}))
	});
}
...
async function handlePost(request: Request) {
	...
	const menu = subscription.menus as { name: string; smart_food_match_enabled: boolean; analytics_reports_enabled: boolean } | null;
	if (menu?.[addon.menusColumn as 'smart_food_match_enabled' | 'analytics_reports_enabled']) {
	...
	try {
		const invoice = await stripe.invoices.create(
			{
				...
				// proration item we just created above and produce an empty,
				// already-"paid" €0 invoice instead of actually charging anything.
				pending_invoice_items_behavior: 'include_and_require',
				metadata: { type: 'addon-midyear', addon: addonKey, subscriptionId: subscription.id }
			},
		...
		// Don't leave an unpaid item sitting on the subscription - it would
		// otherwise still renew (and get billed) again next year.
		await stripe.subscriptionItems.del(item.id, { proration_behavior: 'none' }).catch((rollbackError) => console.error('Rollback failed', rollbackError));
	...
	await Promise.all([
		...
		EMAIL_PATTERN.test(customer.email)
			? sendEmail(
				...
				<p>Berechnet wurde der anteilige Betrag für den Rest Ihres laufenden Abo-Jahres: <strong>${((paid.amount_paid ?? 0) / 100).toFixed(2)} €</strong>. Ab der nächsten Verlängerung läuft es automatisch mit Ihrem Tarif zusammen weiter.</p>
