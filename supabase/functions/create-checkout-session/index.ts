...
Deno.serve(async (request) => {
	...
	try {
		...
		const session = await stripe.checkout.sessions.create({
			...
			subscription_data: {
				metadata: { type: 'initial', plan, firstName, lastName, companyName, phone, email, photoAddon: String(photoAddon), smartFoodMatchAddon: String(smartFoodMatchAddon), analyticsReportsAddon: String(analyticsReportsAddon), smartServiceHubAddon: String(smartServiceHubAddon), lang }
			}
		});
